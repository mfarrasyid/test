 ...
 * 
 */
abstract class REST_Controller extends NoAuthController
{
    ...
    function __construct()
    {
        try {
            ...
            if (array_key_exists('X-Api-Key', $headers) || array_key_exists('X-API-KEY', $headers)) {
                $key = AUTHORIZATION::check_api_key(empty($headers['X-Api-Key']) ? $headers['X-API-KEY'] : $headers['X-Api-Key']);
                if ($key == false) {
                    $this->response(array(
                        'status'   => false,
                    ...
                    $this->load->helper('sec');
                    if (array_key_exists('Authorization', $headers) || array_key_exists('authorization', $headers) && !empty($headers['Authorization'])) {
                        ...
                        // $decodedToken = AUTHORIZATION::validateTimestamp(Sec::decrypt($tkn));
                        $decodedToken = AUTHORIZATION::validateTimestamp($tkn);
                        ...
                        if ($decodedToken == false) {
                            ...
                            // $this->api_data = AUTHORIZATION::validateToken(Sec::decrypt($tkn));
                            $this->api_data = AUTHORIZATION::validateToken($tkn);
                        }
                    } else {
                        $this->response(array(
                            'status'   => false,
                ...
                $this->response(array(
                    'status'   => false,
                    'message'   => "Invalid X-Api-Key or X-API-KEY required."
                ), REST_Controller::HTTP_UNAUTHORIZED);
                return;
            }
        } catch (Exception $err) {
...
class REST_NoAuth extends REST_Controller
{
